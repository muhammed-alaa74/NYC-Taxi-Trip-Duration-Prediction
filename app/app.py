...
with st.sidebar:
    st.markdown("### Ride Configurations")
    ...
    st.markdown("### Map Mode Selection")
...
with col_left:
    if not st.session_state.pickup_coords:
        st.markdown('<div class="instruction-box ins-blue">Welcome! Step 1: Click anywhere on the map to set your <b>Pickup Location</b>.</div>', unsafe_allow_html=True)
    elif not st.session_state.dropoff_coords:
        st.markdown('<div class="instruction-box ins-blue">Pickup Set! Step 2: Now change the mode on the sidebar to <b>"Set Dropoff Location"</b> and click your destination.</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="instruction-box ins-green">Perfect! Your route is ready. Click the <b>Run Model Prediction</b> button on the right to start calculation.</div>', unsafe_allow_html=True)
...
with col_right:
    ...
    if predict_btn:
        if not st.session_state.pickup_coords:
            st.markdown('<div class="instruction-box ins-red">Please drop a pickup pin first.</div>', unsafe_allow_html=True)
        elif not st.session_state.dropoff_coords:
            st.markdown('<div class="instruction-box ins-red">Please drop a dropoff pin first.</div>', unsafe_allow_html=True)
        else:
